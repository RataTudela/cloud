...
export const OrdersPage = () => {
    ...
    const canCreateOrder = isCustomer || isOperator || isAdmin;
    const canChangeStatus = isOperator || isAdmin;
    ...
    const handleStatusChange = async (orderId, currentStatus, newStatus) => {
        if (newStatus === 'DESPACHADO' && !['ACEPTADO', 'EN_PREPARACION'].includes(currentStatus)) {
            setErrorMsg('Regla de Negocio: No se puede cambiar a DESPACHADO sin previa ACEPTACIÓN o PREPARACIÓN del pedido.');
            return;
        }
                    ...
                    <table className="custom-glass-table">
                        ...
                        <tbody>
                            {filteredOrders.length === 0 ? (
                                <tr>
                                    <td colSpan={canChangeStatus ? 6 : 5} className="text-center py-4 text-muted">
